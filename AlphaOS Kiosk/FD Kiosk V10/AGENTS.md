...
- Defaults: `BROWSER=chromium`, `PRIMARY_URL=https://furnituredistributors.net`, `SECONDARY_URL=https://alphaonlines.org/pages/aj-test`, `DEBUG_PORT=9222`, `REBOOT_TIME=03:30`, `PROFILE_DIR=/tmp/kiosk-${BROWSER}`.
...
- UI state lives at `/tmp/kiosk-current-url.txt`; toggle tries Chromium remote-debug first (`http://localhost:$DEBUG_PORT/json/version`) then falls back to that file.
- Browser swap path: `kiosk-ui.py` sends Ctrl+L/type/Enter via `xdotool` to the visible browser window; if none, it spawns `kiosk-session.sh` with `TARGET_URL` set.
- Reboot prompt uses `zenity` and `systemctl reboot` on confirm; if zenity is missing it reboots immediately.
- Dependencies installed via apt in `install.sh`: chromium, firefox-esr, xdotool, wmctrl, unclutter, zenity, python3-tk, dbus-x11, x11-xserver-utils, jq, curl.
- To reconfigure: set env vars (above) and re-run `install.sh`; systemd units are regenerated with the new values.
