...
See `PRODUCTION_CHECKLIST.md` for the full deployment checklist and production config templates.

## Server Install Helper
For server-side install of controller + dashboard + systemd units:
```bash
sudo ./install_server.sh
```

## Docker (Skeleton)
Dockerfiles are provided for controller and dashboard:
- `server/controller/Dockerfile`
- `server/dashboard/Dockerfile`

A basic `docker-compose.yml` is included under `kiosk shell/`.
