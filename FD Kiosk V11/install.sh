#!/usr/bin/env bash
# Version 11.0 installer: deploys kiosk scripts, systemd units, and autostart with xvkbd external keyboard support.
set -euo pipefail
...
SKIP_APT="${SKIP_APT:-0}"
AUTO_APT_SOURCES="${AUTO_APT_SOURCES:-1}"
...
ensure_apt_sources() {
  ...
  local has_deb=0
  if [ -f /etc/apt/sources.list ] && grep -q '^[[:space:]]*deb[[:space:]]' /etc/apt/sources.list 2>/dev/null; then
    has_deb=1
  fi
  if [ "$has_deb" -eq 0 ] && grep -q '^[[:space:]]*deb[[:space:]]' /etc/apt/sources.list.d/*.list 2>/dev/null; then
    has_deb=1
  fi
  if [ "$has_deb" -eq 1 ]; then
    return
  fi
  warn "No active apt sources detected. Package installs will fail."
  warn "Recommended Debian 13 (trixie) sources:"
  warn "deb http://deb.debian.org/debian trixie main contrib non-free non-free-firmware"
  warn "deb http://security.debian.org/debian-security trixie-security main contrib non-free non-free-firmware"
  warn "deb http://deb.debian.org/debian trixie-updates main contrib non-free non-free-firmware"
  if [ "$AUTO_APT_SOURCES" != "0" ]; then
    log "AUTO_APT_SOURCES=1 set; writing /etc/apt/sources.list"
...
install_packages() {
  ...
  if command -v apt >/dev/null 2>&1; then
    ...
    if ! sudo dpkg --configure -a >/dev/null 2>&1; then
      warn "dpkg was interrupted; run 'sudo dpkg --configure -a' if installs fail."
    fi
    sudo apt update
...
enable_units() {
  ...
  sudo -u "$KIOSK_USER" XDG_RUNTIME_DIR="$runtime_dir" DBUS_SESSION_BUS_ADDRESS="$bus_addr" systemctl --user daemon-reload || true
  sudo -u "$KIOSK_USER" XDG_RUNTIME_DIR="$runtime_dir" DBUS_SESSION_BUS_ADDRESS="$bus_addr" systemctl --user enable kiosk.target kiosk-reboot.timer || true
}
  ...
  if [ -f "$src" ]; then
    ...
    case "$desktop_env" in
      ...
      "xfce")
        ...
        sudo -u "$KIOSK_USER" xfconf-query -c xfce4-desktop -p /backdrop/screen0/monitor0/image-path -n -t string -s "/usr/share/backgrounds/alphaos-kiosk.png" || warn "Failed to set XFCE background path"
        sudo -u "$KIOSK_USER" xfconf-query -c xfce4-desktop -p /backdrop/screen0/monitor0/workspace0/last-image -n -t string -s "/usr/share/backgrounds/alphaos-kiosk.png" || warn "Failed to set XFCE workspace background"
        sudo -u "$KIOSK_USER" xfconf-query -c xfce4-desktop -p /backdrop/screen0/monitor0/image-show -n -t bool -s true || warn "Failed to enable XFCE background image"
        # Try to set for monitor 1 if it exists
        sudo -u "$KIOSK_USER" xfconf-query -c xfce4-desktop -p /backdrop/screen0/monitor1/workspace0/last-image -n -t string -s "/usr/share/backgrounds/alphaos-kiosk.png" 2>/dev/null || true
        ;;
      *)
...
main() {
  log "FD Kiosk V11 installer starting"
  ensure_user
  install_packages
  configure_virtual_keyboard
  configure_accessibility
